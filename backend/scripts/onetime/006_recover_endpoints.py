...
# Endpoint
endpoints_file = open('/workspace/backend/scripts/onetime/AlazEndpoint.txt', 'r')
endpoints = json.loads(endpoints_file.read())
batch_num = 0
batch_count = len(endpoints) // batch_size
for batch in process_in_batches(endpoints, batch_size):
    batch_num += 1
    batch_to_insert = []
    for endpoint in batch:
        serializer = EndpointDirectSerializer(data=endpoint)
        if serializer.is_valid():
            batch_to_insert.append(Endpoint(**serializer.validated_data))
        else:
            print(serializer.errors)

    # Attempt bulk insert
    try:
        Endpoint.objects.bulk_create(batch_to_insert)
        ...
        for endpoint in batch_to_insert:
            try:
                endpoint.save()
            except Exception as exc:
                print(f"Error inserting endpoint {endpoint}: {exc}")
endpoints_file.close()
print("Endpoints are added")
