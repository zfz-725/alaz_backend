...
# Service
services_file = open('/workspace/backend/scripts/onetime/AlazService.txt', 'r')
services = json.loads(services_file.read())
batch_num = 0
batch_count = len(services) // batch_size
for batch in process_in_batches(services, batch_size):
    batch_num += 1
    batch_to_insert = []
    for service in batch:
        serializer = ServiceDirectSerializer(data=service)
        if serializer.is_valid():
            batch_to_insert.append(Service(**serializer.validated_data))
        else:
            print(serializer.errors)

    # Attempt bulk insert
    try:
        Service.objects.bulk_create(batch_to_insert)
        ...
        for service in batch_to_insert:
            try:
                service.save()
            except Exception as exc:
                print(f"Error inserting service {service}: {exc}")
services_file.close()
