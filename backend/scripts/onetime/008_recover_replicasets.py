...
# ReplicaSet
replicasets_file = open('/workspace/backend/scripts/onetime/AlazReplicaSet.txt', 'r')
replicasets = json.loads(replicasets_file.read())
batch_num = 0
batch_count = len(replicasets) // batch_size
for batch in process_in_batches(replicasets, batch_size):
    batch_num += 1
    batch_to_insert = []
    for replicaset in batch:
        serializer = ReplicaSetDirectSerializer(data=replicaset)
        if serializer.is_valid():
            batch_to_insert.append(ReplicaSet(**serializer.validated_data))
        else:
            print(serializer.errors)

    # Attempt bulk insert
    try:
        ReplicaSet.objects.bulk_create(batch_to_insert)
        ...
        for replicaset in batch_to_insert:
            try:
                replicaset.save()
            except Exception as exc:
                print(f"Error inserting replicaset {replicaset}: {exc}")
replicasets_file.close()
print("ReplicaSets are added")