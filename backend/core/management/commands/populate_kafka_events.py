...
from core.models import Cluster, KafkaEvent, Pod, ReplicaSet, Request, Service
...
class Command(BaseCommand):
    help = 'Populate Kafka events with fake data'
    ...
    def handle(self, *args, **kwargs):
        ...
        topics = [fake.uri_path() for _ in range(10)]
        partitions = [i for i in range(10)]
        keys = [fake.uri_path() for _ in range(10)]
        values = [fake.uri_path() for _ in range(10)]
        types = ['type1', 'type2']
        ...
        for _ in range(edge_count):
            ...
            for _ in range(req_per_edge):
                ...
                if from_uid_pod:
                    kafka_event.from_uid_deployment = pod_deployments_cache[str(from_uid_pod)]
                    kafka_event.from_uid_daemonset = pods_cache[str(from_uid_pod)].daemonset_owner
                    kafka_event.from_uid_statefulset = pods_cache[str(from_uid_pod)].statefulset_owner
                if to_uid_pod:
                    kafka_event.to_uid_deployment = pod_deployments_cache[str(to_uid_pod)]
                    kafka_event.to_uid_daemonset = pods_cache[str(to_uid_pod)].daemonset_owner
                    kafka_event.to_uid_statefulset = pods_cache[str(to_uid_pod)].statefulset_owner
                    
                bulk_save.append(kafka_event)
                size += 1
                if size == 10000:
                    KafkaEvent.objects.bulk_create(bulk_save)
                    bulk_save = []
                    size = 0
                    flushed += 10000
                    print(f'Flushed {flushed} kafka events in total')
                
        if size > 0:
            KafkaEvent.objects.bulk_create(bulk_save)

        self.stdout.write(self.style.SUCCESS(f'Successfully populated {req_per_edge * edge_count} kafka events.'))